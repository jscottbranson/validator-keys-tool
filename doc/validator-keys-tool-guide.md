...
change if the Xahau config and/or server are compromised.
...
*   sign tokens authorizing a xahaud server to run as the validator identified
    by this public key.
...
The current token needs to be present in the xahaud config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.xahaud/validator-keys.json
```
...
```
  Update xahaud.cfg file with these values:
...
For a new validator, add the [validator_token] value to the xahaud config file.
...
value. After the config is updated, restart xahaud.
...
restart xahaud. Rename the old key file and generate new [validator keys](#validator-keys) and
